...
test.describe
  .serial('project manager creates and approves a time entry for a Customer Project task (UI smoke)', () => {
  ...
  test('creates a Time Entry against the project task on UI and verifies Open status', async ({
    page,
  }) => {
    // Arrange
    ...
    await timeEntry.navigateToNewTimeEntry();

    // Act
    await timeEntry.setEmployee(e.employee.displayName);
    await timeEntry.verifySubsidiaryPrepopulated(e.employeeDefaults.subsidiary.displayName);
    await timeEntry.verifyBillingClassPrepopulated(e.employeeDefaults.billingClass.displayName);
    await timeEntry.verifyDepartmentPrepopulated(e.employeeDefaults.department.displayName);
    await timeEntry.setProject(projectId);
    await timeEntry.verifyBillingSubsidiaryPrepopulated(e.projectDefaults.billingSubsidiary.displayName);
    await timeEntry.verifyJobGroupPrepopulated(e.projectDefaults.jobGroup.displayName);
    await timeEntry.verifyIsBillablePrepopulated(e.projectDefaults.isBillable);
    await timeEntry.setProjectTask(taskId);
    await timeEntry.verifyDefaultRevenueCategoryItemPrepopulated(e.projectTaskDefaults.defaultRevenueCategoryItem.displayName);
    await timeEntry.setServiceItem(e.item);
    await timeEntry.setDate(e.date);
    await timeEntry.setHours(e.hours);
    timeEntryId = await timeEntry.saveTimeEntry(e.employee.id, projectId, taskId);
