...
export class InvoiceRecord extends BasePage {
  ...
  private async ensureFormInited(): Promise<void> {
    await this.page.evaluate(
      () => (globalThis as any).NS?.form?.setInited?.(true),
    );
  }
  ...
  async setSubsidiary(id: string): Promise<void> {
    await this.page.evaluate(
      (v) =>
        (globalThis as any).nlapiSetFieldValue("subsidiary", v, null, true),
      id,
    );
  }

  // Setting the customer (entity) triggers SuiteScript field-change events
  // that populate dependent fields such as Subsidiary — wait for NS to settle.
  // The page reload resets window.isinited, so re-apply after.
  async setCustomer(id: string): Promise<void> {
    await this.page.evaluate(
      (v) => (globalThis as any).nlapiSetFieldValue("entity", v, null, true),
      id,
    );
  ...
  async addLineItem(itemText: string): Promise<void> {
    ...
    await this.page.waitForFunction(
      () => {
        try {
          return !!(globalThis as any).nlapiGetCurrentLineItemValue(
            "item",
            "taxcode",
          );
        } catch {
  ...
  async setLineItemRate(rate: string): Promise<void> {
    ...
    await this.page.waitForFunction(
      () => {
        try {
          const amount = (globalThis as any).nlapiGetCurrentLineItemValue(
            "item",
            "amount",
          );
  ...
  private async setCsegLineItemValue(
    fieldName: string,
    id: string,
  ): Promise<void> {
    await this.page
  ...
  async setOrderedBy(id: string): Promise<void> {
    await this.page.evaluate(
      (v) =>
        (globalThis as any).nlapiSetFieldValue(
          "custbody_eg_ordered_by",
          v,
          null,
          true,
        ),
