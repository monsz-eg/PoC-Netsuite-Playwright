...
export class ProjectRecord extends BasePage {
    ...
    await this.page.evaluate(
      (v) =>
        (globalThis as any).nlapiSetFieldValue(
          'custentity_eg_default_item_auto_ass',
          v,
          null,
          true,
        ),
  ...
  async setServiceItemForTimeBased(id: string): Promise<void> {
    await this.page.evaluate(
      (v) =>
        (globalThis as any).nlapiSetFieldValue(
          'custentity_eg_serviceitemfortimebased',
          v,
          null,
          true,
        ),
  ...
  async setDepartment(id: string): Promise<void> {
    await this.page.evaluate(
      (v) =>
        (globalThis as any).nlapiSetFieldValue('custentity_eg_department_project', v, null, true),
      id,
    );
  }
  ...
  async setCustomerContractPortalAcce(id: string): Promise<void> {
    await this.page.evaluate(
      (v) =>
        (globalThis as any).nlapiSetFieldValue(
          'custentity_eg_customercontractportalacce',
          v,
          null,
          true,
        ),
  ...
  async verifyBillToCustomer(expected: string): Promise<void> {
    await expect(
      this.page.locator('[data-field-name="parent"] [data-nsps-type="field_input"]'),
    ).toHaveText(expected);
  }

  async verifyProjectManager(expected: string): Promise<void> {
    await expect(
      this.page.locator('[data-field-name="projectmanager"] [data-nsps-type="field_input"]'),
    ).toHaveText(expected);
  }
}
