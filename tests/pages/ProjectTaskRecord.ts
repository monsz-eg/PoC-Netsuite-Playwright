...
export class ProjectTaskRecord extends BasePage {
  ...
  async setAssigneeResource(employeeId: string): Promise<void> {
    await this.page.evaluate(
      (id) =>
        (globalThis as any).nlapiSetCurrentLineItemValue('assignee', 'resource', id, null, true),
      employeeId,
    );
  }
  ...
  async setAssigneePlannedWork(hours: string): Promise<void> {
    await this.page.evaluate(
      (h) =>
        (globalThis as any).nlapiSetCurrentLineItemValue('assignee', 'plannedwork', h, null, true),
      hours,
    );
  }
  ...
  async setAssigneeBillingClass(value: string): Promise<void> {
    await this.page.evaluate(
      (v) =>
        (globalThis as any).nlapiSetCurrentLineItemValue('assignee', 'billingclass', v, null, true),
      value,
    );
  }
  ...
  async verifyDefaultItem(expected: string): Promise<void> {
    await expect(
      this.page.locator(
        '[data-field-name="custevent_eg_default_item_auto_ass"] [data-nsps-type="field_input"]',
      ),
    ).toHaveText(expected);
  }
  ...
  async verifyProjectCategory(expected: string): Promise<void> {
    await expect(
      this.page.locator(
        '[data-field-name="custevent_project_category"] [data-nsps-type="field_input"]',
      ),
    ).toHaveText(expected);
  }
