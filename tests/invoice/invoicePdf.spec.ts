...
test('billing responsible can print invoice as PDF in customer locale @smoke', async ({
  ...
  await validatePdfPage(pdfUrl, pdfBody, {
    expectedText: [
      ...
      `${INVOICE_DATA.lineItemPdfNameFixedFee} ${INVOICE_DATA.lineItemDescription} ${INVOICE_DATA.lineItemQuantity} ${formatPdfAmount(INVOICE_DATA.lineItemRate)} ${INVOICE_DATA.lineItemTaxRate} % ${formatPdfAmount(INVOICE_DATA.lineItemRate)}`,
      `${INVOICE_DATA.lineItemPdfNameConsultancyServicesTAndMTraining} ${INVOICE_DATA.lineItemDescription} ${INVOICE_DATA.lineItemQuantity} Time(r) ${formatPdfAmount(INVOICE_DATA.lineItemRate)} ${INVOICE_DATA.lineItemTaxRate} % ${formatPdfAmount(INVOICE_DATA.lineItemRate)}`,
